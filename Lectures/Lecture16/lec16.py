...
Uniform - each result equally probable
    - games, human models
    - natural will not occur

Exponential distributions
    - memory less
    probability to clear drugs - t1 =  1-P
                                 t2 = (1-P)**2
                                 .
                                 .
                                 .
...
choice is not independent of choice of player.
...
def montyChoose(guessDoor, prizeDoor):
    if 1 != guessDoor and 1 != prizeDoor:
        return 1
    if 2 != guessDoor and 2 != guessDoor:
        return 2
    return 3
...
def displayMHSim(simResults):
    ...
    pylab.pie([stickWins, switchWins],colors=['r','g'],labels=['stick','change'],autopct='%.2f%%')
...
def estPi(precision = 0.01, numTrials=20):
    ...
    while sDev >= (precision/4.0):
        ...
        curEst = sum(estimates)/float(len(estimates))
        print 'Est. ='+str(curEst)+'Std. dev. ='+str(sDev)+', Needles ='+str(numNeedles)
        numNeedles *= 2
    return curEst
...
import math
#print integrate(0,8,math.sin,100000)
